---
name: Catoblepas
source: VGM
source_full: "Volo's Guide to Monsters"
aliases: [Catoblepas (VGM)]
tags: [ai_generated, 5etools, monster, monstrosity, cr-5]
type: monster
size: Large
monster_type: monstrosity
alignment: Unaligned
armor_class: 14
hit_points: 84
challenge_rating: 5
xp: 1800
---

# Catoblepas

*Large monstrosity, Unaligned*

---

**Armor Class** 14 (natural armor)  
**Hit Points** 84 (8d10 + 40)  
**Speed** walk 30 ft.

---

| **STR** 19 (+4) | **DEX** 12 (+1) | **CON** 21 (+5) | **INT** 3 (-4) | **WIS** 14 (+2) | **CHA** 8 (-1) |

---

**Saving Throws** —  
**Skills** —  
**Damage Resistances** —  
**Damage Immunities** —  
**Condition Immunities** —  
**Senses** darkvision 60 ft., passive Perception 12  
**Languages** —  
**Challenge** 5 (1,800 XP)

---

## Traits

**Keen Smell.** The catoblepas has advantage on Wisdom ([[Perception]]) checks that rely on smell.

**Stench.** Any creature other than a catoblepas that starts its turn within 10 feet of the catoblepas must succeed on a DC 16 Constitution saving throw or be [[poisoned]] until the start of the creature's next turn. On a successful saving throw, the creature is immune to the stench of any catoblepas for 1 hour.


## Actions

**Tail.** Melee Weapon Attack +7 to hit, reach 10 ft., one target. @h21 (**5d6 + 4**) bludgeoning damage, and the target must succeed on a DC 16 Constitution saving throw or be [[stunned]] until the start of the catoblepas's next turn.

**Death Ray {@recharge 5}.** The catoblepas targets a creature that it can see within 30 feet of it. The target must make a DC 16 Constitution saving throw, taking 36 (**8d8**) necrotic damage on a failed save, or half as much damage on a successful one. If the saving throw fails by 5 or more, the target instead takes 64 necrotic damage. The target dies if reduced to 0 hit points by this ray.






---

**Source:** Volo's Guide to Monsters, p. 129
